...
class SaroController extends Controller
{
    public function addSaro(Request $request)
    {
        ...
        try {
            DB::connection('dtc')->table('saro')->insert([
    ...
    {
        $query = DB::connection('dtc')->table('saro')->orderBy('year', 'desc');
    ...
    public function saveNTCA(Request $request)
    {
        ...
        try {
            ...
            $existingNTCA = DB::connection('dtc')->table('ntca')->where('ntca_no', $validatedData['ntca_no'])->first();
            ...
            if ($existingNTCA) {
                ...
                $saro = DB::connection('dtc')->table('saro')->where('saro_no', $validatedData['saro_no'])->first();
                ...
                DB::connection('dtc')->table('ntca')->insert([
    ...
    {
        try {
            $ntca = DB::connection('dtc')->table('ntca')->where('ntca_no', $ntcaNo)->first();
    ...
    {
        try {
            $ntcaRecords = DB::connection('dtc')->table('ntca')->where('saro_no', $saroNo)->get();
            ...
            foreach ($ntcaRecords as $ntca) {
                $ntca->saro_budget = DB::connection('ilcdb')->table('saro')->where('saro_no', $ntca->saro_no)->value('budget_allocated'); // From saro table
                $ntca->ntca_budget = DB::connection('ilcdb')->table('ntca')->where('ntca_no', $ntca->ntca_no)->value('budget_allocated'); // From ntca table
            }
