...
class OtherExpenseFormController extends Controller
{
    ...
    public function updateotherexpense(Request $request)
    {
        $validatedData = $request->validate([
            'procurement_id' => 'required|exists:spark.otherexpense_form,procurement_id',
        ...
        try {
            ...
            $record = DB::connection('spark')->table('otherexpense_form')
            ...
            DB::connection('spark')->transaction(function () use ($validatedData, $status, $unit, $record) {
                ...
                DB::connection('spark')->table('otherexpense_form')
                    ...
                    DB::connection('spark')->table('procurement')
                        ->where('procurement_id', $validatedData['procurement_id'])
                        ->update([
                            'ntca_no' => $validatedData['ntca_no'] ?? null,
                            'quarter' => $validatedData['quarter'] ?? null,
    ...
    public function uploadedFilesCheck($procurement_id)
    {
        ...
        $uploadedFiles = DB::connection('spark')->table('requirements')
    ...
    public function upload(Request $request)
    {
        try {
            ...
            $uploadedFiles = DB::connection('spark')
                ->table('requirements')
                ->where('procurement_id', $request->procurement_id)
                ->get();
    ...
    {
        try {
            $files = DB::connection('spark')->table('requirements')
                ->where('procurement_id', $procurement_id)
                ->get();
