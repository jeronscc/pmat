...
function fetchProcurementForSaro(saroNo) {
    const url =
        saroNo === ""
            ...
            : `/api/spark/fetch-procurement-spark?saro_no=${saroNo}`;
            ...
            if (data.length > 0) {
                ...
                data.sort((a, b) => {
                    ...
                    const statusComparison =
                        (statusOrder[statusA] || 5) -
                        (statusOrder[statusB] || 5);
                ...
                data.forEach((item) => {
                    ...
                    categoryCell.textContent =
                        item.procurement_category || "N/A"; // Add category cell
...
function fetchProcurementForYear(year) {
    const url =
        year === ""
            ? "/api/spark/fetch-procurement-spark"
            : `/api/spark/fetch-procurement-spark?year=${year}`;
    ...
    fetch(url)
        ...
        .then((data) => {
            ...
            if (data.length > 0) {
                ...
                document
                    .querySelectorAll("#procurementTable tr")
                    .forEach((row) => {
                ...
                emptyCell.setAttribute("colspan", "3");
                emptyCell.textContent =
                    "No procurement records found for the selected year.";
                emptyMessage.appendChild(emptyCell);
                tableBodies.all.appendChild(emptyMessage);
            }
        })
...
function openProcurementModal(item) {
    ...
    const url = `/api/spark/fetch-procurement-details?procurement_id=${procurementId}`;
...
function fetchProcurementData(year = "", status = "all") {
    let url = "/api/spark/fetch-combined-procurement-data";
